...
urlpatterns = [
    ...
    path("<int:board_pk>/comments", views.Comments.as_view(), name="comments-creation"),  # 특정 게시글과 관련 댓글에 접근할 수 있는 url
    path("<int:board_pk>/comments/<int:comment_pk>", views.CommentsDetail.as_view(), name="comments-detail"),  # 특정 댓글에 접근할 수 있는 url")
    path("<int:board_pk>/comments/<int:comment_pk>/thumb-up", views.ThumbUp.as_view(), name="thumb-up"),  # 댓글 추천에 접근할 수 있는 url
]
